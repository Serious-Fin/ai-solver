...
export async function getSession(sessionId: string): Promise<SessionInfo> {
	try {
		const response = await fetch(`${getBackendHost()}/session/${sessionId}`)
		if (!response.ok) {
...
async function tryGetExistingUser(userId: string): Promise<User | undefined> {
	try {
		const response = await fetch(`${getBackendHost()}/user/${userId}`)
		if (!response.ok) {
			if (response.status === 404) {
				return undefined
			}
...
async function createNewUser(user: User): Promise<void> {
	try {
		const response = await fetch(`${getBackendHost()}/user`, {
			method: 'POST',
			headers: {
...
async function startSession(userId: string): Promise<string> {
	try {
		const response = await fetch(`${getBackendHost()}/session`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
